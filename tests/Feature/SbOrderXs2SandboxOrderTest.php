...
class SbOrderXs2SandboxOrderTest extends TestCase
{
    ...
    public function test_preview_xs2_order_includes_main_listing_when_ticket_resolved_via_event_match(): void
    {
        $event = Xs2Event::query()->create([
            'external_event_id' => 'preview-event-fallback',
        ...
        $this->withToken($this->adminToken())
            ->getJson("/api/admin/sb-orders/{$sbOrder->id}/preview-xs2-order")
            ->assertOk()
            ->assertJsonPath('data.skip_reason', 'SB order must be Pending Confirmation to create an XS2 order (current: Cancelled).')
            ->assertJsonPath('data.reservation_request.items.0.ticket_id', $ticket->external_ticket_id)
            ->assertJsonPath('data.reservation_request.items.0.net_rate', 6863)
            ->assertJsonPath('data.reservation_request.items.0.currency_code', 'EUR')
            ->assertJsonPath('data.main_listing.xs2_ticket_id', $ticket->id)
            ->assertJsonPath('data.main_listing.external_ticket_id', $ticket->external_ticket_id)
            ->assertJsonPath('data.main_listing.original_price', 68.63)
            ->assertJsonPath('data.main_listing.original_currency', 'EUR');
    }
    ...
    public function test_manual_create_endpoint_links_existing_synced_order_without_ticket_mapping(): void
    {
        app(IntegrationSettingService::class)->set(
            ApiEnvironmentService::XS2_ORDERS_ACTIVE_ENVIRONMENT,
            ApiEnvironmentService::ENV_PRODUCTION,
        ...
        $this->withToken($this->adminToken())
            ...
            ->assertOk()
            ->assertJsonPath('data.id', $sbOrder->id)
            ->assertJsonPath('data.xs2_order.xs2_booking_id', '7d4c92fa89544519bc81d664c09c1c39_bkn')
            ->assertJsonPath('data.xs2_order.external_order_id', '7d4c92fa89544519bc81d664c09c1c39_bko');
    ...
    public function test_create_manual_finds_synced_order_by_booking_reference_not_recent_scan(): void
    {
        app(IntegrationSettingService::class)->set(
            ApiEnvironmentService::XS2_ORDERS_ACTIVE_ENVIRONMENT,
            ApiEnvironmentService::ENV_PRODUCTION,
        ...
        $syncedOrder = Xs2Order::query()->create([
            ...
            'synced_at' => now()->subDays(30),
        ]);

        for ($i = 0; $i < 105; $i++) {
            Xs2Order::query()->create([
                'external_order_id' => 'filler-order-'.$i.'_bko',
                'is_sandbox' => false,
                'sb_order_id' => null,
                'xs2_booking_id' => 'filler-booking-'.$i.'_bkn',
                'order_status' => 'completed',
                'raw_payload' => ['booking_reference' => 'FILLER-'.$i],
                'synced_at' => now(),
            ]);
        }
    ...
    public function test_create_manual_links_synced_order_when_pending_row_was_deleted(): void
    {
        app(IntegrationSettingService::class)->set(
            ApiEnvironmentService::XS2_ORDERS_ACTIVE_ENVIRONMENT,
            ApiEnvironmentService::ENV_PRODUCTION,
    ...
    public function test_create_manual_links_synced_order_when_booking_reference_only_in_raw_payload_text(): void
    {
        app(IntegrationSettingService::class)->set(
            ApiEnvironmentService::XS2_ORDERS_ACTIVE_ENVIRONMENT,
            ApiEnvironmentService::ENV_PRODUCTION,
        ...
        $syncedOrder = Xs2Order::query()->create([
            ...
            'order_status' => 'completed',
            'raw_payload' => [
                'notes' => 'SeatsBroker SB order 1BX67678',
                'booking_id' => '7d4c92fa89544519bc81d664c09c1c39_bkn',
            ],
            'synced_at' => now(),
        ]);
        ...
        $this->assertFalse($result['skipped']);
        $this->assertSame($syncedOrder->id, $result['order']?->id);
        $this->assertDatabaseMissing('xs2_orders', ['external_order_id' => 'sb-pending:1BX67678']);
        Http::assertNothingSent();
    }

    public function test_create_manual_links_synced_order_by_event_name_and_date(): void
    {
        app(IntegrationSettingService::class)->set(
            ApiEnvironmentService::XS2_ORDERS_ACTIVE_ENVIRONMENT,
            ApiEnvironmentService::ENV_PRODUCTION,
    ...
    public function test_create_manual_links_on_api_failure_instead_of_creating_pending(): void
    {
        ...
        Http::fake([
            'https://api.xs2.test/v1/reservations' => Http::response([
                'message' => 'Local XS2 rate limit reached',
            ], 422),
        ]);

        $event = Xs2Event::query()->create([
            'external_event_id' => 'production-event-roma-rate-limit',
            'event_name' => 'AS Roma vs Atalanta',
            'sport_type' => 'soccer',
            'event_status' => 'closed',
            'date_start_local' => '2026-09-05 20:45:00',
        ...
        Xs2Ticket::query()->create([
            'external_ticket_id' => 'production-ticket-roma-rate_tck',
            'external_event_id' => $event->external_event_id,
            'xs2_event_id' => $event->id,
            'is_sandbox' => false,
            'ticket_status' => 'available',
    ...
    public function test_create_manual_links_synced_order_by_fuzzy_event_name(): void
    {
        app(IntegrationSettingService::class)->set(
            ApiEnvironmentService::XS2_ORDERS_ACTIVE_ENVIRONMENT,
            ApiEnvironmentService::ENV_PRODUCTION,
    ...
    public function test_manual_create_endpoint_returns_linked_message(): void
    {
        ...
        Xs2Order::query()->create([
            ...
            'event_date' => '2026-09-05',
            'order_status' => 'completed',
            'raw_payload' => ['booking_reference' => '1BX67678'],
            'synced_at' => now(),
        ]);
        ...
        $this->withToken($this->adminToken())
            ->postJson("/api/admin/sb-orders/{$sbOrder->id}/create-xs2-order")
            ->assertOk()
            ->assertJsonPath('message', 'Linked to existing XS2 production order 7d4c92fa89544519bc81d664c09c1c39_bko for booking 1BX67678.');
