...
class EnableSellerListing implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(SellerApiClient $client): void
    {
        ...
        if (! $mapping || ! $mapping->seller_listing_id || $mapping->status === 'active') {
            return;
        }
        ...
        try {
            $response = $client->enableListing($mapping->seller_listing_id, $payload);
            $mapping->update(['status' => 'active', 'last_request' => $payload, 'last_response' => $response, 'last_error' => null, 'disabled_at' => null]);
            Log::channel(config('services.seller_api.log_channel', 'stack'))->info('Seller listing enabled.', [
                'ticket_id' => $ticket->id,
            ...
            Log::channel(config('services.seller_api.log_channel', 'stack'))->error('Seller listing enable failed.', [
                'ticket_id' => $ticket->id,
                'seller_listing_id' => $mapping->seller_listing_id,
                'request' => $payload,
                'error' => $e->getMessage(),
