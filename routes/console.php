...
// honour Start All / Stop All without requiring a PHP worker restart.
$schedulerEnabled = fn (): bool => (bool) config('app.scheduler_enabled', true);
...
Schedule::command('xs2:sync-inventory --mode=incremental')
    ...
    ->when(fn (): bool => $xs2Enabled() && now()->minute !== 0)
...
if ($eventsSyncScheduled) {
    Schedule::command('xs2:sync-events')
        ...
        ->when(fn (): bool => $xs2Enabled() && now()->hour !== 0)
        ->withoutOverlapping($overlapMinutes)
        ->onOneServer();
...
Schedule::command('xs2:sync-order-guest-data')
    ...
    ->when(fn (): bool => $xs2Enabled() && (bool) config('xs2.sb_order_guest_data_sync.enabled', true))
    ->withoutOverlapping($overlapMinutes)
    ->onOneServer();
...
Schedule::command('xs2:sync-sb-listing-inventory')
    ...
    ->when(fn (): bool => $sellerApiEnabled() && (bool) config('xs2.sb_listing_inventory.enabled', true))
    ->withoutOverlapping($overlapMinutes)
    ->onOneServer();
...
$sbPublishSchedule
    ->when(fn (): bool => $sellerApiEnabled() && (bool) config('xs2.sb_new_listing_publish.enabled', true))
    ->withoutOverlapping($overlapMinutes)
    ->onOneServer();
...
$sbBookingsSchedule
    ->when(fn (): bool => $sellerApiEnabled() && (bool) config('xs2.sb_bookings_sync.enabled', true))
    ->withoutOverlapping($overlapMinutes)
    ->onOneServer();

Schedule::command('sanctum:prune-expired --hours=24')
    ->daily()
    ->when($schedulerEnabled)
    ->withoutOverlapping($overlapMinutes)
    ->onOneServer();
